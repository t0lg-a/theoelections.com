...
// Pollster allowlists. Mirror forecast.js GB_AP / APPROVAL_AP exactly.
// Generic ballot: 20 pollsters, no YouGov/Ipsos.
const GB_AP = [
  /verasight/, /americanresearchgroup|arg\b/, /tipp/, /emerson/, /gallup/,
  /marist/, /quinnipiac/, /apnorc|ap\-norc|norc/, /marquette/, /cnnssrs|cnn\/ssrs|ssrs/,
  /atlasintel|atlas/, /beaconresearch|shaw/, /hartresearch|publicopinionstrategies/,
  /pewresearch|pew/, /surveymonkey/, /leger/, /massachusetts|umass|departmentofpoliticalscience/,
  /siena|newyorktimes/, /foxnews/, /wallstreetjournal|wsj/,
];
// Approval: 22 pollsters, GB list + YouGov + Ipsos.
const APPROVAL_AP = [/yougov/, /ipsos/, ...GB_AP];

function normPollster(s) {
  return String(s || "").toLowerCase().replace(/&/g, "and").replace(/[^a-z0-9]+/g, "");
}
function isAllowed(pollster, allowlist) {
  const n = normPollster(pollster);
  if (!n) return false;
  return allowlist.some((re) => re.test(n));
}
function filterByAllowlist(polls, allowlist, label) {
  const before = polls.length;
  const dropped = new Set();
  const kept = polls.filter((p) => {
    const ok = isAllowed(p && p.pollster, allowlist);
    if (!ok && p && p.pollster) dropped.add(p.pollster);
    return ok;
  });
  console.log(`${label}: filter ${before} -> ${kept.length} (dropped ${before - kept.length} polls across ${dropped.size} pollsters)`);
  return { kept, droppedPollsters: Array.from(dropped).sort() };
}
...
async function run() {
  ...
  try {
    ...
    console.log("Fetching Generic Ballot...");
    {
      ...
      const json = await fetchJson(url);
      const rawGb = extractList(json);
      console.log(`Generic Ballot: Found ${rawGb.length} polls.`);
      const gbFiltered = filterByAllowlist(rawGb, GB_AP, "Generic Ballot");
      data.genericBallot = gbFiltered.kept;
      data.debug.gbDroppedPollsters = gbFiltered.droppedPollsters;
    }
    ...
    const dedupedApproval = dedupeById(all).sort((a, b) => String(b.end_date).localeCompare(String(a.end_date)));
    console.log(`Approval: Found ${dedupedApproval.length} polls (after dedupe).`);
    const appFiltered = filterByAllowlist(dedupedApproval, APPROVAL_AP, "Approval");
    data.approval = appFiltered.kept;
    data.debug.approvalDroppedPollsters = appFiltered.droppedPollsters;
